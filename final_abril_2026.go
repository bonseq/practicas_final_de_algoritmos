...
//Acada espacio = 0
ALGORITMO SECUENCIA FINAL ES
AMBIENTE
FUNCION ENCRIP(CARACTER); ENTERO
	i:=0
	MIENTRAS i<28 O A[i]=V HACER
		i:=i+1
	FIN MIENTRAS
	SI i<28 entonces
		ENCRIP:=i
	SINO
		W("SE ENCONTRO UN CARCATER DESCONOCIDO, NO IRA A LA SALIDA")
	FS
FIN FUNCION
	
PROCESO
W("INGRESE PESO ELALFABETO PARA DARLE PESO, ESTE DEBE DE SER DE 27 LETRAS COMO EL CONVENCIONAL")
PARA i:=1 A 27 HACER
	R(CARACTER)
	A(i):=caracter
FIN PARA
crea(salida,s), abrir(sec,v)
MINTRAS NFDS(SEC) ENTONCES 
	PRIM:=nil
	MIENTRAS V <> ' ' HACER
		NUEVO (p)
    	*P.DATO:=ENCRIP(V)
    	*P.PROX:=nil
		...
		AVZ(SEC,V)
	FIN MIENTRAS
	SI RESGUARDO_V EN VOCALES ENTONCES
		*a.DATO:=*a.DATO+RaizCuadrada(*a.DATO)
	SINO
		*a.DATO:=*a.DATO+Factorial(*a.DATO)
	FIN SINO 
...
FUNCION TIEMPO_RATA(N:ENTERO):ENTERO ES
    SI N=3 ENTONCES
		TIEMPO_RATA:=TIEMPO_RATA+7
	SINO	
		SI N=2 ENTONCES
			TIEMPO_RATA:=TIEMPO_RATA(RAMDOM)+5
		SINO 
			SI N=1 ENRONCES
				TIEMPO_RATA:=TIEMPO_RATA(RAMDOM)+3
			FIN SI
		FIN SINO
	FIN SINO
FIN FUNCION
...
PROCESO
W("EL CAMINO QUE LA RATA ELIJA ESTA DADA POR LA FUNCION RAMDOM")

TIEMPO:=TIEMPO_RATA(RAMDOM())

W("EL TIEMPO Q LA RATA TARDO ES DE", TIEMPO, "MINUTOS")
